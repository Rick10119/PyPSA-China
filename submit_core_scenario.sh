...
# circumvent snakemake iocacheStartup failure caused by missing files
...
# Submit only specified core scenarios
ALLOWED_JOBS=(
...
for job_file in jobs/job_*.slurm; do
    if [ -f "$job_file" ]; then
        ...
        if [[ " ${ALLOWED_JOBS[*]} " == *" ${job_basename} "* ]]; then
            JOBS+=("$job_file")
            echo "  ✓ Discover job files: ${job_basename}"
        fi
    fi
done
